...
import { CreatePollValidationSchema, UpdatePollValidationSchema, CreateAnswerValidationSchema, StartPollSchema } from '@schemas/poll.schema';
...
export class PollRoute implements Routes {
  ...
  private initializeRoutes() {
    ...
    this.router.delete(`${this.path}/:id`, AuthMiddleware, RoleMiddleware(['TEACHER']), this.pollController.deletePoll);
    ...
    this.router.post(`${this.path}/answer`, AuthMiddleware, ValidationMiddleware(CreateAnswerValidationSchema), this.pollController.submitAnswer);

    this.router.get(`${this.path}/active`, AuthMiddleware, RoleMiddleware(['TEACHER']), this.pollController.getActivePoll);
